...
//        }
        ...
        if($current_user->ID == 0 && $no_login_needed_for_booking == "yes" && isset($_REQUEST['new_reser_exp_request_user_email'])) {
...
if( !function_exists('homey_add_instance_exp_booking') ) {
    function homey_add_instance_exp_booking($experience_id, $check_in_date, $guests, $renter_message, $extra_options, $user_id = null) {
        global $current_user;
        $current_user = wp_get_current_user();
        $userID       = $current_user->ID;
...
if( !function_exists('homey_calculate_exp_reservation_cost_guestly') ) {
    function homey_calculate_exp_reservation_cost_guestly($reservation_id, $collapse = false) { // homey_calculate_exp_reservation_cost_personly => homey_calculate_exp_reservation_cost_guestly
        ...
        $sub_total_amnt = $upfront_payment - $city_fee -  $security_deposit - $services_fee - $taxes;
...
if(!function_exists('homey_get_exp_prices')) {
    function homey_get_exp_prices($check_in_date, $experience_id, $guests, $extra_options = null) {
        ...
        $enable_taxes = homey_option('enable_exp_taxes');
        $offsite_payment = homey_option('off-site-payment');
        $reservation_payment_type = homey_option('exp_reservation_payment');
        $booking_percent = homey_option('exp_booking_percent');
        $tax_type = homey_option('exp_tax_type');
        $apply_taxes_on_service_fee  =   homey_option('apply_exp_taxes_on_service_fee');
        $taxes_percent_global  =   homey_option('exp_taxes_percent');
        $single_experience_tax = get_post_meta($experience_id, 'homey_exp_tax_rate', true);
...
//             echo ' the person => price='. $returnPrice.'<br>';
        ...
        $experience_host_id = get_post_field( 'post_author', $experience_id );
        $host_reservation_payment_type = get_user_meta($experience_host_id, 'host_reservation_payment', true);
        $host_booking_percent = get_user_meta($experience_host_id, 'host_booking_percent', true);
...
if(!function_exists('homey_instant_exp_booking')) {
    function homey_instant_exp_booking() {
        ...
        if($no_login_needed_for_booking == 'no' && $userID == $experience_owner_id) {
            echo json_encode(
                array(
                    'success' => false,
                    'message' => $local['own_experience_error']
...
if(!function_exists('check_exp_booking_availability')) {
    function check_exp_booking_availability($check_in_date, $experience_id, $guests) {
        $return_array = array();
        $local = homey_get_localization();
        $booking_proceed = true;
        ...
        if ( (array_key_exists($check_in_unix, $reservation_booked_array) && ($remaining_no_of_attendee < 1 || $remaining_no_of_attendee < $guests) ) || (array_key_exists($check_in_unix, $reservation_pending_array) && ($remaining_no_of_attendee < 1 || $remaining_no_of_attendee < $guests)  ) || array_key_exists($check_in_unix, $reservation_unavailable_array)) {
