...
if(!function_exists('homeyGenerateMonth')) {
	function homeyGenerateMonth( $numberOfMonths, $unixMonth, $currentMonth, $currentYear ) {
		...
		foreach ( $weekArray as $weekDay ) {
			...
			$dayName = (true == $weekDayInitial) ? $wp_locale->get_weekday_abbrev($weekDay) : $wp_locale->get_weekday_abbrev($weekDay);
			$weekDays .= '<li data-dayName = "'.esc_attr($weekDay).'">'.  esc_attr( esc_html__($dayName, 'homey') ).'</li>';
		}
