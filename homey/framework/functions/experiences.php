...
if( !function_exists('homey_get_experience_data')) {
    function homey_get_experience_data($field) {
        $prefix = 'homey_';
...
if( !function_exists('homey_experience_filter_callback') ) {
    function homey_experience_filter_callback( $lsiting_qry ) {
        ...
        $experiences_booking_type = get_post_meta( $page_id, $prefix.'experiences_booking_type', true );
        if( !empty($experiences_booking_type) ) {
            $meta_query[] = array(
                'key'     => $prefix.'booking_type',
                'value'   => $experiences_booking_type,
                'compare' => '=',
                'type'    => 'CHAR'
            );
        }
...
if( !function_exists('homey_exp_stripe_payment_for_featured') ) {
    function homey_exp_stripe_payment_for_featured() {
        ...
        $description = esc_html__( 'Upgrade to Featured, Lisiting ID','homey').' '.$experience_id;
...
if( !function_exists('homey_exp_stripe_payment_for_featured_old') ) {
    function homey_exp_stripe_payment_for_featured_old() {
        ...
        $experience_id     = isset($_GET['upgrade_id']) ? $_GET['upgrade_id'] : '';
        $upfront_payment = floatval( homey_option('price_featured_experience') );
        ...
        <input type="hidden" id="experience_id" name="experience_id" value="' . $experience_id . '">
...
if ( !function_exists( 'homey_loadmore_experiences' ) ) {
    function homey_loadmore_experiences() {
        ...
        $experience_roomtype = homey_traverse_comma_string($roomtype);
        ...
        if (!empty($experience_roomtype)) {
            $tax_query[] = array(
                'taxonomy' => 'room_type',
                'field' => 'slug',
                'terms' => $experience_roomtype
            );
        }
...
if( !function_exists('homey_exp_header_map') ) {
    function homey_exp_header_map() {
        ...
        while( $query_args->have_posts() ): $query_args->the_post();
            ...
            $bedrooms       = get_post_meta( get_the_ID(), $prefix.'experience_bedrooms', true );
            ...
            $experience->price = homey_formatted_price($experience_price, false, true).'<sub>'.esc_attr($price_separator).homey_get_price_label_by_id($experience_id).'</sub>';
            $experience->address = $address;
            $experience->bedrooms = $bedrooms;
            $experience->guests = $guests;
            $experience->beds = $beds;
            $experience->baths = $baths;
            if($cgl_types != 1) {
            ...
            if( has_post_thumbnail( $experience_id ) ) {
                $experience->thumbnail = get_the_post_thumbnail( $experience_id, 'homey-experience-thumb',  array('class' => 'img-responsive' ) );
            }else{
                $experience->thumbnail = homey_get_image_placeholder( 'homey-experience-thumb' );
            }
...
if( !function_exists('homey_exp_sticky_map') ) {
    function homey_exp_sticky_map() {
        $local = homey_get_localization();
        $tax_query = array();
        $meta_query = array();
        $experiences = array();
...
//             if(($edit_experiences_admin_approved != 0) && !homey_is_admin()) {
        ...
        if( $experience_id > 0 ) {
            ...
            if( isset( $_POST['additional_rules'] ) ) {
                update_post_meta( $experience_id, $prefix.'additional_rules', sanitize_textarea_field( $_POST['additional_rules'] ) );
            }
            ...
            if( isset( $_POST['what_to_provided'] ) ) {
                ...
                $what_to_provided = array_filter($what_to_provided, function($v){
                    return !empty($v['name']) && !empty($v['desc']);
                });

                if( isset( $what_to_provided[0]['name'] ) ) {
...
// validate Email
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_save_as_draft', 'save_experience_as_draft');
if( !function_exists('save_experience_as_draft') ) {
    function save_experience_as_draft() {
        ...
        if( isset( $_POST['description'] ) ) {
            $new_experience['post_content'] = wp_kses_post( $_POST['description'] );
        }
        ...
        if( $experience_id > 0 ) {
            ...
            // Bedrooms
            if( isset( $_POST['experience_bedrooms'] ) ) {
                update_post_meta( $experience_id, $prefix.'experience_bedrooms', sanitize_text_field( $_POST['experience_bedrooms'] ) );
            }
            ...
            // Beds
            if( isset( $_POST['beds'] ) ) {
                update_post_meta( $experience_id, $prefix.'beds', sanitize_text_field( $_POST['beds'] ) );
            }

            // Baths
            if( isset( $_POST['baths'] ) ) {
                update_post_meta( $experience_id, $prefix.'baths', sanitize_text_field( $_POST['baths'] ) );
            }
            ...
            // Additional Rules
            if( isset( $_POST['additional_rules'] ) ) {
                update_post_meta( $experience_id, $prefix.'additional_rules', sanitize_text_field( $_POST['additional_rules'] ) );
            }
...
if( !function_exists('homey_experience_sort')) {
    function homey_experience_sort($query_args) {
        ...
        if ( isset( $_GET['sortby'] ) ) {
            ...
            if ( is_page_template( array( 'template/template-experience-list.php', 'template/template-experience-grid.php', 'template/template-experience-card.php', 'template/template-experience-sticky-map.php' ))) {
                ...
                $sort_by = get_post_meta( get_the_ID(), 'homey_experiences_halfmap_sort', true );
...
if( !function_exists( 'homey_experience_gallery_upload' ) ) {
    function homey_experience_gallery_upload( ) {
        ...
        if ( isset( $uploaded_image['file'] ) && $is_dimension_valid != -1 ) {
            ...
            $experience_thumb = wp_get_attachment_image_src( $attach_id, 'homey-experience-thumb' );
...
if( !function_exists('homey_create_experience_print')) {
    function homey_create_experience_print () {
        ...
        $bedrooms   = homey_get_experience_data_by_id('experience_bedrooms', $experience_id);
        $beds       = homey_get_experience_data_by_id('beds', $experience_id);
        ...
        if($booking_type == 'per_hour') {
            $weekends_price = homey_get_experience_data_by_id('hourly_weekends_price', $experience_id);
        } else {
        ...
        <div class="print-main-wrap">
            <div class="print-wrap">
                        ...
                        <div class="block-head">
                            <div class="media">
                                ...
                                <div class="media-body">
                                    <h2 class="title"><?php echo homey_option('sn_hosted_by'); ?> <span><?php echo esc_attr($experience_author['name']); ?></span></h2>

                                    <?php if(!empty($experience_author['city'])) { ?>
                                    <address><i class="fa fa-map-marker" aria-hidden="true"></i> <?php echo esc_attr($experience_author['city']); ?></address>
                                    <?php } ?>
                                ...
                                <div class="block-right">
                                    <ul class="detail-list detail-list-2-cols">
                                        ...
                                        <li>
                                            ...
                                            <?php echo esc_attr(homey_option('sn_id_label')); ?>: <strong><?php echo esc_attr($experience_id); ?></strong>
                                        </li> 
                                        <?php } ?>
                                        ...
                                        <?php if( (!empty($room_type) || !empty($experience_type)) && $hide_labels['sn_type_label'] != 1  ) { ?>
                                        <li><i class="fa fa-angle-right" aria-hidden="true"></i> 
                                            <?php echo homey_option('sn_type_label'); ?>: <strong><?php echo esc_attr($room_type).' '.$slash.' '.esc_attr($experience_type); ?></strong>
                                        </li>
                                        <?php } ?>
                ...
                <?php
                $accomodation = homey_get_experience_data_by_id('accomodation', $experience_id);
