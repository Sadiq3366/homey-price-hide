...
$termCountryIds = get_terms([
    'taxonomy' => 'listing_country',
    'name__like' => $keyword,
    'fields' => 'ids'
]);

$termCityIds = get_terms([
    'taxonomy' => 'listing_city',
    'name__like' => $keyword,
    'fields' => 'ids'
]);

$termStateIds = get_terms([
    'taxonomy' => 'listing_state',
    'name__like' => $keyword,
    'fields' => 'ids'
]);

$termAreaIds = get_terms([
    'taxonomy' => 'listing_area',
    'name__like' => $keyword,
    'fields' => 'ids'
]);
...
//  load the posts using the found term IDs
    ...
    if(!empty($termCountryIds) || !empty($termCityIds) || !empty($termStateIds) || !empty($termAreaIds)){
        $query_args['tax_query'] = [
                'relation' => 'OR',
        ];

        if(!empty($termCountryIds)){
            $query_args['tax_query'][] =    [
                'taxonomy' => 'listing_country',
                'field' => 'id',
                'terms' => $termCountryIds,
            ];
        }

        if(!empty($termCityIds)){
            $query_args['tax_query'][] =    [
                'taxonomy' => 'listing_city',
                'field' => 'id',
                'terms' => $termCityIds,
     ...
     $query_args['s'] = str_replace('-', ' ', $keyword);
    }
