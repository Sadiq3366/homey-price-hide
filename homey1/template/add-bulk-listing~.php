...
function mycallADD()
{
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        // 6th page options
        $_POST['homey_accomodation']  = $data[39];
        ...
        //-----ical
        $_POST['ical_feed_name']  = $data[54];
        $_POST['ical_feed_url']   = $data[55];
        //

        //-----custom price
        $_POST['custom_price_date_in']      = explode(',', $data[56]);
        ...
        listing_submission_filter($new_listing);

        //----- Custom-Bulk ---- Save listing custom periods
        if( isset( $_POST['custom_price_date_in'] ) && !empty( $_POST['custom_price_date_in'][0] ) )
        {
